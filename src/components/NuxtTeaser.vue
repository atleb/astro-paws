...
// Computed property for meta flags
...
const teaserClasses = computed(() => {
  ...
  if (props.customProperties?.frontPageCardSize) {
    classes.push(String(props.customProperties.frontPageCardSize));
  }
...
<template>
  ...
  <a :href="`https://www.fvn.no/i/${id}`" class="teaser-link-wrapper">
    ...
    <div :class="teaserClasses">
      ...
      <img v-if="hasImage" :src="mediaUrl(media.imageAsset.id)" />
    </div>
  </a>
</template>

<style>
/* Colors for section-specific borders: */
.section-sport {
...
.teaser-link-wrapper {
  display: block;
  /* Ensures the link takes up the full width of its container */
  color: inherit;
  /* Inherits text color from parent, preventing default link blue */
  text-decoration: none;
  /* Removes default underline from the link */
...
:root {
  ...
  /* Default border radius for teasers and images */
  --teaser-padding: 0.75em;
  /* Base padding unit for teaser containers */
  --teaser-margin-bottom: 1.5em;
  /* Space below each teaser */

  --color-border-default: #e0e0e0;
  /* Default border color for teasers */
  --color-border-medium: #dcdcdc;
  /* Border color for medium-sized teasers */
  --color-border-small: #e0e0e0;
  /* Border color for small-sized teasers (consistent with default) */

  --color-breaking-news-bg: #fff5f5;
  /* Background color for breaking news teasers */
  --color-breaking-news-border: red;
  /* Border color for breaking news teasers */
  --color-breaking-news-text: red;
  /* Text color for breaking news specific elements (e.g., "VARSKO!" heading) */

  --font-size-title: 1.2rem;
  /* Font size for the main title (h3) */
  --font-weight-title: 600;
  /* Font weight for the main title */
  --font-size-lead: 0.9rem;
  /* Font size for the lead text paragraph */
  --line-height-lead: 1.4;
  /* Line height for the lead text for better readability */
  --font-size-breaking-heading: 1.1em;
  /* Font size for the "VARSKO!" heading in breaking news */
}
...
.teaser-container {
  ...
  width: 100%;
  /* Explicitly take full width for mobile-first */
  box-sizing: border-box;
  /* Include padding and border in the element's total width and height */
  padding: var(--teaser-padding);
  /* Uses the base padding unit */
  ...
  /* Ensures background color for content and shadow visibility */
...
.no-image {
  grid-template-areas:
    ...
    "b b";
  /* Lead text now spans full width */
...
.teaser-link-wrapper:focus-visible .teaser-container {
  ...
  box-shadow: 0 0 0 2px #005fcc, 0 6px 12px rgba(0, 0, 0, 0.15);
  /* Blue focus ring and slightly enhanced shadow */
...
.breaking-news {
  background-color: var(--color-breaking-news-bg);
  /* Distinct background for breaking news */
  border: 2px solid var(--color-breaking-news-border);
  /* Prominent border for breaking news */
  box-shadow: 0 2px 5px rgba(200, 0, 0, 0.1);
  /* Optional: themed shadow for breaking news */
}

/* Styles for the "VARSKO!" heading in breaking news */
.breaking-news-heading {
  color: var(--color-breaking-news-text);
  /* Uses breaking news text color */
  font-weight: bold;
  margin-bottom: 0.5em;
  font-size: var(--font-size-breaking-heading);
  /* Uses specific font size for this heading */
  letter-spacing: 0.5px;
  /* Adds letter-spacing for emphasis */
}
...
.medium {
  ...
  /* Specific border color for medium cards */
  padding: var(--teaser-padding);
  /* Consistent padding */
  display: grid;
  grid-template-areas:
    ...
    /* Title area */
    "b c c";
  /* Lead text area (b) and Image area (c) */
  grid-template-rows: auto 1fr;
  /* Title row height based on content, second row takes remaining space */
  grid-template-columns: 1fr 100px 100px;
  /* Defines column widths, image area is last two columns */
  gap: 0.5em;
...
.medium h3 {
  font-size: 1.1rem;
  /* Overrides default title font-size for medium cards */
}
...
.small {
  border: 1px solid var(--color-border-small);
  /* Specific border color for small cards */
  padding: calc(var(--teaser-padding) * 0.8);
  /* Slightly reduced padding for smaller cards */
  display: grid;
  grid-template-areas:
    "a a "
    /* Title area */
    "b c";
  /* Lead text area (b) and Image area (c) - image takes less relative space */
  gap: 0.3em;
  /* Smaller gap for compact layout */
  grid-template-rows: auto 1fr;
  /* Title row auto, content row takes remaining space */
}
...
.medium img {
  max-width: 200px;
  /* Maximum width for the image in medium cards */
}

.small img {
  max-width: 100px;
  /* Maximum width for the image in small cards - adjusted to be "tiny" */
}

/* Specific title styling for small variant */
.small h3 {
  font-size: 0.95rem;
  /* Overrides default title font-size for small cards */
}

/* Adjust lead text size for .small teasers to improve hierarchy with title */
.small .lead {
  font-size: 0.8rem;
  /* Reduced from default var(--font-size-lead) which is 0.9rem */
...
h3 {
  ...
  /* Default font size; can be overridden by .medium h3, .small h3 */
  font-weight: var(--font-weight-title);
  /* Uses defined font weight variable */
  overflow: hidden;
  /* Necessary for text-overflow to work */
  ...
  grid-area: a;
  /* Needs to be explicit for revised style... */
}

/* General styling for the lead text paragraph */
.lead {
  font-size: var(--font-size-lead);
  /* Uses defined font size variable */
  line-height: var(--line-height-lead);
  /* Uses defined line height variable */
  margin: 0 0 0.5em 0;
  /* Small bottom margin */
  overflow: hidden;
  /* Necessary for text-overflow */
  text-overflow: ellipsis;
  /* Adds ellipsis for overflowing text */
  ...
  grid-area: b;
  /* baseline for revised */
}

.teaser-container:not(.medium):not(.small) img {
  width: 100%;
  /* Image takes full width in default layout */
  height: auto;
...
.tag-count-badge {
  ...
  margin-bottom: 0.5em;
  /* Space below the badge if image is not present */
  line-height: 1;
  /* Ensure line height doesn't add extra space */
}
...
.author-count-badge {
  ...
  /* Space above the badge */
  margin-left: 0.5em;
  /* Space to the left if tag badge is also present */
  margin-bottom: 0.5em;
  /* Space below the badge if image is not present */
  line-height: 1;
  /* Ensure line height doesn't add extra space */
